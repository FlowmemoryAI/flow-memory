...
export function RLPanel({ state }: { state: VisualNetworkState }) {
  ...
  const beforeAfter = episode?.before_after;
  return (
    <section className="panel panel-rl">
      ...
      {episode ? (
        <dl>
          ...
          <div><dt>Before/after</dt><dd>{beforeAfter ? `${beforeAfter.before ?? "?"} → ${beforeAfter.after ?? "?"}` : "prototype metric unavailable"}</dd></div>
        </dl>
      ) : <p>No RL episode in this visual state. RL suggestions remain advisory.</p>}
    </section>
  );
}
