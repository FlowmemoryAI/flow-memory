...
Agent Builder read endpoints require `x-flow-memory-scopes: agent-builder:read`; plan and birth require `x-flow-memory-scopes: agent-builder:create`; upgrade simulation requires `x-flow-memory-scopes: agent-builder:simulate`.
...
For local development, `npm run dev` serves the real Mission Control page at `http://127.0.0.1:4173/mission-control` and the Agent Builder builder at `http://127.0.0.1:4173/agents/new`. It renders checked-in replay/mock fixtures without the local API: the run selector, Flow Memory Agent Builder, Live Neural Agent Launch, Live Agent Operations, Live Agent Supervisor, Local Network Replay, Predictive Cognition panel, Predictive Learning Benchmark panel, Agent Genesis panel, Proof of Learning panel, Neural Embodiment panel, Live 3D Mode panel, GPU evidence status, and public-alpha finalizer status.

Agent Builder shows a simple first-agent path where the first agent requires no wallet/API key/funds. Private memory is default, network learning is opt-in, and BYOK, wallet identity, on-chain dry-run, and x402 dry-run routes are optional advanced capabilities after birth.
...
- **Flow Memory Agent Builder** — `dashboard/src/mock-data/agent-builder.json`
