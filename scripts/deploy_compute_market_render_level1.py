...
def require_public_observability_sinks(
    ...
    if missing:
        emit(
            ...
            missing_values=missing,
            required_action=(
                "configure authenticated HTTPS alert webhook, error tracking webhook, and OTLP collector "
                "sinks before public Level 1 deployment"
            ),
        )
    placeholders = [
        key
        ...
        if value.strip() and has_placeholder(value)
    ]
    if placeholders:
        emit(
            "blocked_placeholder_observability_credentials",
            29,
            placeholder_values=placeholders,
            required_action="replace public observability sink credentials with real secrets before deployment",
        )
...
def build_env_vars(
    ...
    values = {
        ...
        "FLOW_MEMORY_API_KEY_SCOPES": "api:read api:write api:admin api:audit compute:read compute:plan compute:execute compute:admin compute:audit compute:provider-admin compute:policy-admin compute:billing compute:settlement-admin inference:read inference:plan inference:proxy inference:buy inference:sell inference:admin inference:audit",
        "FLOW_MEMORY_API_REQUIRE_SCOPES": "true",
    ...
    *,
    ...
    ok = all(
        (
            ...
            safety.get("dry_run_required") is True,
            safety.get("live_settlement_enabled") is False,
            safety.get("broadcast_enabled") is False,
            safety.get("private_key_inputs_allowed") is False,
            safety.get("audit_required") is True,
            safety.get("audit_export_required") is True,
            (not require_immutable_audit or safety.get("audit_export_immutable_required") is True),
            ...
            "idempotency_nonunique_indexes" in schema_verification,
            not schema_verification.get("idempotency_nonunique_indexes", ()),
    ...
    return {
        ...
        "dry_run_required": safety.get("dry_run_required"),
        "live_settlement_enabled": safety.get("live_settlement_enabled"),
        "broadcast_enabled_readiness": safety.get("broadcast_enabled"),
        "private_key_inputs_allowed": safety.get("private_key_inputs_allowed"),
...
def main() -> int:
    ...
    require_public_observability_sinks(
        alert_webhook_url,
        error_tracking_webhook_url,
        otlp_endpoint_url,
        alert_webhook_secret,
        error_tracking_webhook_secret,
        otlp_headers,
    )
