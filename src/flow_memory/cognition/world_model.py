...
class DeterministicWorldModel:
    ...
    def encode_state(self, payload: Mapping[str, Any] | None = None, *, root: str | Path = ".") -> WorldState:
        ...
        if not memory_context and similar:
    ...
    def predict_outcome(self, state: WorldState, action: CandidateAction, memories: tuple[Mapping[str, Any], ...] = ()) -> PredictionRecord:
        ...
        return PredictionRecord(
            ...
            memory_support_ids=tuple(str(item.get("experience_id", item.get("memory_id", ""))) for item in memories if item.get("experience_id") or item.get("memory_id")),
        )
    ...
    def score_candidates(self, actions: tuple[CandidateAction, ...], counterfactuals: CounterfactualSet, memories: tuple[Mapping[str, Any], ...] = ()) -> tuple[CandidateScore, ...]:
        return score_candidates(actions, counterfactuals.candidate_predictions, memory_support=min(1.0, len(memories) / 5.0))
    ...
    def tick(self, payload: Mapping[str, Any] | None = None, *, root: str | Path = ".") -> Mapping[str, Any]:
        ...
        memories = retrieve_similar_experiences(state, root=root, limit=5)
        ...
        recommended_id = counterfactuals.recommended_action_id
        ...
        experience = build_experience(
            state=state,
            retrieved_memory_ids=tuple(str(item.get("experience_id", "")) for item in memories if item.get("experience_id")),
...
def _domain(goal: str) -> str:
    if "dashboard" in goal or "mission" in goal:
        return "dashboard"
    if "release" in goal or "public-alpha" in goal:
        return "release"
...
def _reward_for(action: CandidateAction, state: WorldState, memory_support: float) -> float:
    ...
    if action.action_type in {"diagnostic", "simulation"}:
        reward += 0.10
    if action.expected_domain in {"dashboard", "release"}:
        reward += 0.08
