...
def migration_plan() -> Mapping[str, Any]:
    return {
        ...
        "steps": (
            {
                "version": COMPUTE_MARKET_STORAGE_VERSION,
                "name": "compute_market_production_records_v3_audit_chain",
                ...
                "description": "Create durable typed JSON record storage, production query indexes, and tamper-evident audit chain columns for Flow Memory Compute Market.",
                "reversible": False,
                "managed_sql_notes": (
                    "The schema uses portable SQL types: text, integer, and JSON-serialized payload text.",
                    "PostgreSQL deployments should map payload to jsonb and keep the same primary key plus chain indexes.",
                    "Run migrations before serving traffic and restore from a database snapshot for rollback.",
                ),
            },
        ),
    }
