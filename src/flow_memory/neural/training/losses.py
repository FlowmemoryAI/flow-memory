...
def predictive_latent_loss(predicted, target):
    torch = require_torch()
    return torch.mean((predicted - target) ** 2)


def temporal_consistency_loss(tokens):
    torch = require_torch()
    if tokens.shape[1] < 2:
...
def total_dual_stream_loss(predicted, target, dorsal_a, dorsal_b):
    return predictive_latent_loss(predicted, target) + 0.25 * appearance_suppression_loss(dorsal_a, dorsal_b)


def total_agent_policy_loss(success_pred, success_target, risk_pred, risk_target):
