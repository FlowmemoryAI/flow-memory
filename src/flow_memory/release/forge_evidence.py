"""Release evidence for Flow Memory Forge browser agent builder."""
...
def flow_memory_forge_evidence(root: str | Path = ".") -> Mapping[str, object]:
    root_path = Path(root).resolve()
    with TemporaryDirectory() as tmp:
        defaults = forge_defaults()
        plan = create_forge_assembly_plan({"name": "Mira", "purpose": "Help me build Flow Memory"}, root=tmp)
        birth = birth_agent_from_forge({"name": "Mira", "purpose": "Help me build Flow Memory"}, root=tmp)
        upgrades = simulate_forge_upgrades(str(birth["agent_id"]), byok=True, wallet=True, onchain_dry_run=True, x402=True, root=tmp)
    ...
    dashboard_fixture = root_path / "dashboard" / "src" / "mock-data" / "flow-memory-forge.json"
    plan_record = dict(plan.get("plan", {}))
    evidence = {
        "forge_available": _file_contains(root_path / "src" / "flow_memory" / "forge" / "core.py", "ForgeAgentAssemblyPlan"),
        "forge_browser_route_available": _file_contains(dashboard_server, "id=\"forge\"") and _file_contains(dashboard_server, "url.pathname === '/forge'"),
        "forge_first_agent_simple_mode_available": defaults.get("simple_mode_default") is True and plan_record.get("first_agent_mode") is True,
        "forge_advanced_mode_available": any(card.get("optional") for card in defaults.get("capability_cards", ())),
        ...
        "skill_matcher_from_forge_available": bool(upgrades.get("ok")) and _file_contains(dashboard_server, "Find collaborators"),
        "mission_control_handoff_available": str(birth.get("mission_control_url", "")).startswith("/mission-control#forge"),
        "read_only_demo_mode_available": dashboard_fixture.exists() and _file_contains(dashboard_server, "read-only demo mode"),
        "cli_forge_available": _file_contains(root_path / "src" / "flow_memory" / "cli.py", "def _forge") and _file_contains(root_path / "src" / "flow_memory" / "cli.py", '"forge"'),
        "api_forge_available": _REQUIRED_ROUTES.issubset(manifest_routes) and _REQUIRED_ROUTES.issubset(registered_routes) and _scope_checks_ok(),
        "mermaid_forge_docs_available": "```mermaid" in docs_text and "forge architecture" in docs_text,
        ...
        "public_alpha_docs_updated": all((root_path / path).exists() for path in ("docs/FLOW_MEMORY_FORGE.md", "docs/AGENT_GENESIS.md", "docs/AGENT_INTERNET.md")) and "first agent requires no wallet/api key/funds" in docs_text,
    ...
    return {
        ...
        **evidence,
        ...
        "artifact_paths": {
            "forge_plans": "artifacts/forge/plans/",
            "births": "artifacts/genesis/births/",
            "capability_upgrades": "artifacts/capability_upgrades/",
            "dashboard_fixture": "dashboard/src/mock-data/flow-memory-forge.json",
        },
    }


def verify_flow_memory_forge_evidence(record: Mapping[str, object]) -> Mapping[str, object]:
    blockers: list[str] = []
    required = (
        "forge_available",
        "forge_browser_route_available",
        "forge_first_agent_simple_mode_available",
        "forge_advanced_mode_available",
        ...
        "skill_matcher_from_forge_available",
        "mission_control_handoff_available",
        "read_only_demo_mode_available",
        "cli_forge_available",
        "api_forge_available",
        "mermaid_forge_docs_available",
    ...
    if record.get("ok") is not True:
        blockers.append("forge_evidence_not_ok")
    for key in required:
...
def _scope_checks_ok() -> bool:
    return (
        required_scopes_for("GET", "/forge/defaults") == (FORGE_READ_SCOPE,)
        and required_scopes_for("POST", "/forge/assembly-plan") == (FORGE_CREATE_SCOPE,)
        and required_scopes_for("POST", "/forge/birth") == (FORGE_CREATE_SCOPE,)
        and required_scopes_for("POST", "/forge/simulate-upgrades") == (FORGE_SIMULATE_SCOPE,)
    )
