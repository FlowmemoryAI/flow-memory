...
def test_torch_policy_status_imports_without_torch():
    status = torch_policy_status()
    assert status["backend"] == "torch"
    assert status["available"] in {True, False}


def test_torch_policy_constructor_requires_torch_when_absent():
...
def test_torch_policy_smoke_skips_without_torch_or_trains_with_torch():
