...
vec3 strandColor(float t) {
    vec3 amber = vec3(0.82, 0.58, 0.32);
    vec3 blue = vec3(0.10, 0.42, 1.00);
    vec3 memory = vec3(0.86, 0.92, 1.00);
    return t < 0.5 ? mix(amber, blue, t * 2.0) : mix(blue, memory, (t - 0.5) * 2.0);
}
...
void addStrand(inout vec3 color, inout float alpha, vec2 p, float id) {
    ...
    float width = mix(0.0022, 0.0064, hash(id * 8.1));
    vec3 accum = vec3(0.0);
    float strand = 0.0;
    ...
    float pulseT = fract(uTime * mix(0.030, 0.085, hash(id * 3.9)) + hash(id * 5.4));
    vec2 pulsePos = bezier(a, b, c, d, pulseT);
    float pulse = smoothstep(0.035, 0.0, length(p - pulsePos));
    float weight = 0.28 + 0.50 * smoothstep(0.18, 0.0, abs(pulseT - 0.50));

    color += accum * (0.09 + weight * 0.12);
    color += strandColor(pulseT) * pulse * 0.85;
    alpha += strand * 0.018 + pulse * 0.42;
}
...
float knot(vec2 p, float id) {
    ...
    float l = length(p);
    float target = r + sin(a * (2.0 + id) + uTime * (0.18 + id * 0.03)) * 0.038;
...
void main() {
    ...
    vec3 color = vec3(0.018, 0.028, 0.045);
    float alpha = 1.0;

    float vignette = smoothstep(1.35, 0.15, length(p));
    color += vec3(0.015, 0.035, 0.070) * vignette;
    ...
    for (int i = 0; i < 7; i++) {
        ...
        color += mix(knotColor, vec3(0.92, 0.96, 1.0), float(i) / 7.0) * k * 0.42;
        alpha += k * 0.18;
    }
    ...
    for (int i = 0; i < 80; i++) {
        ...
        q.x += sin(uTime * 0.025 + float(i)) * 0.018;
        float dotGlow = smoothstep(0.010, 0.0, length(p - q));
        color += vec3(0.50, 0.68, 1.0) * dotGlow * 0.18;
    }

    color *= 0.86 + vignette * 0.32;
